...
class WaveClipperOne extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    if (!reverse && !flip) {
      Offset firstEndPoint = Offset(size.width * .8, size.height - 190);
      Offset firstControlPoint = Offset(size.width * .25, size.height - 0);
      Offset secondEndPoint = Offset(size.width, size.height - 50);
      Offset secondControlPoint = Offset(size.width * .175, size.height - 100);
      ...
      Offset firstControlPoint = Offset(size.width * .10, size.height - 5);
      Offset secondEndPoint = Offset(size.width, size.height - 5);
      Offset secondControlPoint = Offset(size.width * .95, size.height - 50);

      final path = Path()
        ..lineTo(0.0, size.height - 70)
        ..quadraticBezierTo(firstControlPoint.dx, firstControlPoint.dy,
            firstEndPoint.dx, firstEndPoint.dy)
        ..quadraticBezierTo(secondControlPoint.dx, secondControlPoint.dy,
            secondEndPoint.dx, secondEndPoint.dy)
